...
class SunriseAndSunsetExceptionHandlerTest {
  ...
  void testNoResourceFoundException() {
    SunriseAndSunsetExceptionHandler exceptionHandler = new SunriseAndSunsetExceptionHandler();
    NoResourceFoundException e = new NoResourceFoundException(HttpMethod.GET, "/nonexistent-path");
    ResponseEntity<SunriseAndSunsetExceptionHandler.Message> response = exceptionHandler.noResourceFoundException(e);

    assertEquals(HttpStatus.NOT_FOUND, response.getStatusCode());
    assertEquals("ERROR 404: No Resource Found", response.getBody().getMessage());
    assertEquals("No static resource /nonexistent-path.", response.getBody().getDescription());
  }
